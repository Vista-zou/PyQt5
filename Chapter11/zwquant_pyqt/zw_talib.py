...
def MACD(df, n_fast, n_slow,ksgn='close'):
    ...
    EMAslow = df[ksgn].ewm(span=n_slow, min_periods=n_slow - 1).mean() #pd.Series(pd.ewma(df[ksgn], span = n_slow, min_periods = n_slow - 1))
    MACD = pd.Series(EMAfast - EMAslow, name=xnam)
    MACDsign = pd.Series(MACD.ewm(span=9, min_periods=8).mean(), name=xnam2)
